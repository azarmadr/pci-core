...
static unsigned int intx = 0; // module parameter, requested irq number
...
*/	
	...
	if(intx){ // request specific IRQ line	
		pdev->irq = intx;
		if (request_irq(intx, irq_handler, IRQF_SHARED, "pmc_wb", dev) < 0) {
			printk(KERN_ALERT PMC_WB ": could not register requested interrupt handler %d\n", pdev->irq);
			goto fail_reg;
		}
	}
	else{
		if (request_irq(pdev->irq, irq_handler, IRQF_SHARED, "pmc_wb", dev) < 0) {
			printk(KERN_ALERT PMC_WB ": could not register assigned interrupt handler %d\n", pdev->irq);
			goto fail_reg;
		}
	}
...
MODULE_PARM_DESC(intx, "0 - default INTx ; X - requested INTx");
MODULE_LICENSE("GPL");
